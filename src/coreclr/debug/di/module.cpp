...
HRESULT CordbNativeCode::GetReturnValueLiveOffsetImpl(Instantiation *currentInstantiation, ULONG32 ILoffset, ULONG32 bufferSize, ULONG32 *pFetched, ULONG32 *pOffsets)
{
    ...
    // Use the new mechanism: scan NativeVarInfo for CALL_RETURN_ILNUM entries
    // whose callReturnValueILOffset matches the requested IL offset.
    // The startOffset of such entries directly gives the native offset where the
    // return value is live, without needing architecture-specific disassembly.
    const DacDbiArrayList<ICorDebugInfo::NativeVarInfo> *pVarInfoList = m_nativeVarData.GetOffsetInfoList();

    if (pVarInfoList != NULL)
    {
        for (unsigned int i = 0; i < pVarInfoList->Count(); i++)
        {
            const ICorDebugInfo::NativeVarInfo *pVarInfo = &((*pVarInfoList)[i]);

            if (pVarInfo->varNumber == (DWORD)ICorDebugInfo::CALL_RETURN_ILNUM &&
                pVarInfo->callReturnValueILOffset == ILoffset)
            {
                if (pOffsets && found < bufferSize)
                {
                    pOffsets[found] = pVarInfo->startOffset;
                }
                found++;
            }
        }
