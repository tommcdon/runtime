        ...
        if ((contextBufSize != 0) && (pbContextBuf != NULL))
        {
            ...
            if (m_fIsOneFrameAhead)
            {
                if (m_pCachedFrame != NULL)
                {
                    ...
                    printf("CordbStackWalk::GetContext reading m_pCachedFrame's context Flags=0x%08x RIP=%p, RSP=%p, RBP=%p, RAX=%p\n", 
                           pSrcContext->ContextFlags, pSrcContext->Rip, pSrcContext->Rsp, pSrcContext->Rbp, pSrcContext->Rax);
                    fflush(stdout);
                    _ASSERTE(pSrcContext);
                    CORDbgCopyThreadContext(pContext, pSrcContext);
                    printf("CordbStackWalk::GetContext copy of m_pCachedFrame's context Flags=0x%08x RIP=%p, RSP=%p, RBP=%p, RAX=%p\n", 
                           pContext->ContextFlags, pContext->Rip, pContext->Rsp, pContext->Rbp, pContext->Rax);
                    fflush(stdout);
                }
                else
                {
