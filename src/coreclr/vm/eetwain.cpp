...
static void VirtualUnwindInterpreterCallFrame(TADDR sp, T_CONTEXT *pContext)
{
    ...
    if (pFrame != NULL)
    {
        // The interpreter saves pFrame->ip as the resumption point AFTER the CALL/CALLVIRT instruction.
        // Subtract 1 to point back into the call instruction's native offset range, analogous to how
        // JIT return addresses naturally fall within the call instruction. This ensures that
        // MapNativeOffsetToIL resolves to the call site's source line, not the next statement.
