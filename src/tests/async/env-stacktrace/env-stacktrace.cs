...
public class Async2EnvStackTrace
{
    ...
    /// <summary>
    /// Validates stitching across a v2 -> v1 -> v2 boundary. A classic (v1) async method sits
    /// between two runtime-async (v2) methods, mirroring an ASP.NET pipeline whose framework
    /// methods are classic async. The v1 method's frame must be recovered from its
    /// AsyncStateMachineBox in the waiter chain.
    /// </summary>
    [Fact]
    public static void TestV1BoundaryStitching()
    {
        V1BoundaryEntry().GetAwaiter().GetResult();
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    [RuntimeAsyncMethodGeneration(false)]
    private static async Task V1BoundaryEntry()
    {
        string trace = await V2OuterAsync();

        Assert.True(
            trace.Contains(nameof(V2InnerAsync), StringComparison.Ordinal),
            "Expected trace to contain v2 inner " + nameof(V2InnerAsync) + "." + Environment.NewLine + trace);

        Assert.True(
            trace.Contains(nameof(V1MiddlewareAsync), StringComparison.Ordinal),
            "Expected trace to contain v1 middleware " + nameof(V1MiddlewareAsync) + " (recovered across v2->v1 boundary)." + Environment.NewLine + trace);

        Assert.True(
            trace.Contains(nameof(V2OuterAsync), StringComparison.Ordinal),
            "Expected trace to contain v2 outer " + nameof(V2OuterAsync) + "." + Environment.NewLine + trace);
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static async Task<string> V2OuterAsync()
    {
        return await V1MiddlewareAsync();
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    [RuntimeAsyncMethodGeneration(false)]
    private static async Task<string> V1MiddlewareAsync()
    {
        return await V2InnerAsync();
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    private static async Task<string> V2InnerAsync()
    {
        await Task.Delay(1);
        return Environment.AsyncStackTrace;
    }
