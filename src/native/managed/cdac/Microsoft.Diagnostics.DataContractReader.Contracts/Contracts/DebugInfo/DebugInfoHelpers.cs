...
internal static class DebugInfoHelpers
{
    ...
    internal static IEnumerable<DebugVarInfo> DoVars(NativeReader nativeReader, bool isX86)
    {
        ...
        const uint CALL_RETURN_ILNUM = unchecked((uint)-6);

        for (uint i = 0; i < varCount; i++)
        {
            // New encoding: varNumber comes first so field #2 can be conditional
            uint varNumber = reader.ReadUInt() + MAX_ILNUM;
            uint startOffset = reader.ReadUInt();

            uint endOffset;
            uint returnValueILOffset = 0;

            if (varNumber == CALL_RETURN_ILNUM)
            {
                // For return value homes, field #2 is the IL offset of the call instruction
                returnValueILOffset = reader.ReadUInt();
                endOffset = startOffset + 1;
            }
            else
            {
                // For normal variables, field #2 is end offset delta
            ...
            yield return locType switch
            {
                ...
                {
                    StartOffset = startOffset, EndOffset = endOffset, VarNumber = varNumber, ReturnValueILOffset = returnValueILOffset,
                ...
                VarLocType.VLT_FPSTK => ConsumeAndDefault(reader.ReadUInt(), startOffset, endOffset, varNumber, returnValueILOffset),
                VarLocType.VLT_FIXED_VA => ConsumeAndDefault(reader.ReadUInt(), startOffset, endOffset, varNumber, returnValueILOffset),
                _ => new DebugVarInfo
                {
                    StartOffset = startOffset, EndOffset = endOffset, VarNumber = varNumber, ReturnValueILOffset = returnValueILOffset,
                },
    ...
    private static DebugVarInfo ConsumeAndDefault(uint _, uint startOffset, uint endOffset, uint varNumber, uint returnValueILOffset) => new()
    {
        StartOffset = startOffset, EndOffset = endOffset, VarNumber = varNumber, ReturnValueILOffset = returnValueILOffset,
    };
