...
internal partial struct RuntimeTypeSystem_1 : IRuntimeTypeSystem
{
    ...
    public IEnumerable<TargetPointer> GetFieldDescs(TypeHandle typeHandle)
    {
        if (!typeHandle.IsMethodTable())
            yield break;

        TargetPointer fieldDescListPtr = GetFieldDescList(typeHandle);
        ushort numInstanceFields = GetNumInstanceFields(typeHandle);
        ushort numStaticFields = GetNumStaticFields(typeHandle);

        TypeHandle parentTypeHandle = GetTypeHandle(GetParentMethodTable(typeHandle));
        ushort numParentInstanceFields = GetNumInstanceFields(parentTypeHandle);

        // numInstanceFields contains both fields declared on this type and inherited fields
        // the fieldDescList contains only the fields declared on this type, not inherited fields
        int fieldsDeclaredOnThisType = numStaticFields + numInstanceFields - numParentInstanceFields;
        uint fieldDescSize = _target.GetTypeInfo(DataType.FieldDesc).Size ?? throw new InvalidOperationException("Unable to get FieldDesc size");
        for (int i = 0; i < fieldsDeclaredOnThisType; i++)
        {
    ...
    private MetadataReader? LookForHandle(AssemblyReference exportedAssemblyRef)
    {
        ...
        foreach (ModuleHandle mdhandle in _target.Contracts.Loader.GetModuleHandles(
            appDomain,
